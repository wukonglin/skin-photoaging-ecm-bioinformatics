...
# -*- coding: utf-8 -*-
"""
17_figure_round3_public_rationale.py  --  ROUND 3 (PI feedback): public-data-only Figure 1
==========================================================================================
Round-3 design feedback: the front/rationale figure must read as a TRUE pre-experiment
argument built ONLY from public databases/literature. No downstream experimental data result and no
protective third arm may appear here -- the first figure compares normal vs photoaged skin and
nominates the ECM/collagen remodeling axis as the design target. (The downstream experimental data
validation is split into a separate later/supplementary figure, generated by script 18.)

Panels (schematic-led composite):
  A  Public-data rationale workflow (schematic; public datasets -> axis nomination -> material design)
  B  GSE173385 scRNA, NORMAL vs PHOTOAGED only: cell-class UMAP (fibroblasts highlighted) +
     ECM/collagen module-score UMAP + fibroblast ECM/collagen module score (exploratory, cell-level)
  C  Pathway enrichment of photoaging-suppressed fibroblast genes (ECM/collagen organization) -- the
     statistical hero panel; pure public-data evidence
  D  Literature-curated human skin aging/photoaging matrisome: broad structural decline, by category
  E  Design-rationale schematic (axis -> SFMP-HA-SF hydrogel design; downstream validation roadmap)

NOTHING from the downstream experimental data enters this figure. Wording uses "nominated / design
rationale / to be validated", never post-experiment verbs.

  conda run -n junye_ecm python 04_code/17_figure_round3_public_rationale.py

Inputs (all already on disk; nothing re-downloaded, nothing fabricated):
...
Outputs:
  08_figures/revision_round_3_PI_feedback_20260612/Figure1_public_ECM_rationale.pdf / .png
"""
...
# ---------- style (nature-figure python quick-start: editable PDF text, restrained palette) ----------
plt.rcParams.update({
...
# Restrained palette: one neutral/blue "disease/loss" family + one red "up" accent.
# The treated-sample signal green is intentionally absent (no experimental data from this study here).
NORMAL_C, PHOTO_C = "#3b6fb0", "#e6731c"      # normal (control) vs photoaged (UVB)
PUB_DOWN, PUB_UP  = "#4575b4", "#d73027"      # public aging: down / up
...
OUT=ROOT/"08_figures"/"revision_round_3_PI_feedback_20260612"; OUT.mkdir(parents=True,exist_ok=True)

# ---------- matrisome categorisation (Naba-style, finer per spec) -- identical to round-2 main ----------
...
# Figure 1 scope: normal (Control) vs photoaged (UV) only. The protective third arm of the public
# dataset is intentionally excluded -- this study does not evaluate that arm, so showing it would
# raise an out-of-scope reviewer question and break the clean normal-vs-disease rationale.
...
# ---------- Panel A: public-data rationale workflow (schematic; public datasets only) ----------
...
um=adB.obsm["X_umap"]
# B-i cell classes, fibroblast highlighted
...
# B-iii fibroblast ECM module: normal vs photoaged (exploratory, cell-level)
...
_wbb=dict(boxstyle="round,pad=0.2",fc="white",ec="none",alpha=0.80)
axB3.annotate(f"$\\Delta$mean = {muv-mctrl:+.2f}\n({muv:.2f} vs {mctrl:.2f})",(0.03,0.985),xycoords="axes fraction",
              fontsize=7.0,fontweight="bold",color="#333",ha="left",va="top",bbox=_wbb)
axB3.annotate(f"exploratory: cell-level only,\n1 sample / condition (MWU p={pcell:.1e})",(0.03,0.86),
              xycoords="axes fraction",fontsize=5.9,style="italic",color="#666",ha="left",va="top",
              linespacing=0.95,bbox=_wbb)

# ---------- Panel C: enrichment of photoaging-suppressed fibroblast genes (statistical hero) ----------
...
# ---------- Panel D: public human matrisome decline by category ----------
...
# ---------- Panel E: design-rationale schematic (NOT a result) ----------
...
axE.text(0.5,0.99,"Design rationale  (how the public-nominated axis motivates the material -- not a Figure 1 result)",
         ha="center",va="top",fontsize=8.2,style="italic",color="#555")
# 1) nominated axis  -> 2) material design (with 3 feature chips) -> 3) validation roadmap
box(axE,0.010,0.40,0.215,0.34,"Public-data-nominated\nECM-collagen\nremodeling axis",BOX_AXIS,fs=8.0,bold=True)
arr(axE,0.227,0.300,0.57)
box(axE,0.300,0.40,0.235,0.34,"SFMP-HA-SF hydrogel\nis designed to rebuild an\nECM-mimetic adhesive\ninterface",BOX_MAT,fs=7.8,bold=True)
arr(axE,0.537,0.700,0.57)
box(axE,0.700,0.40,0.290,0.34,"To be validated in this study:\ncollagen I / III / IV, fibronectin,\nvinculin, histological collagen\nremodeling",BOX_VAL,fs=7.6)
# feature chips under the material box
chips=[("HA","hydration &\ndermal filling"),
       ("Silk fibroin","bioactive ECM-like\npolymer matrix"),
       ("SF microspheres","microscale adhesive\ninterfaces for cells")]
cw=0.112
for i,(h,t) in enumerate(chips):
    x=0.300+i*0.118
    axE.add_patch(FancyBboxPatch((x,0.06),cw,0.26,boxstyle="round,pad=0.006,rounding_size=0.025",
                                 fc="#fbf1e7",ec="#caa57f",lw=0.8))
    axE.text(x+cw/2,0.27,h,ha="center",va="top",fontsize=6.6,fontweight="bold",color="#9c5a22")
    axE.text(x+cw/2,0.185,t,ha="center",va="center",fontsize=6.0,color="#7a4a22",linespacing=0.95)
    axE.add_patch(FancyArrowPatch((x+cw/2,0.40),(x+cw/2,0.325),arrowstyle="-",lw=0.7,color="#caa57f",linestyle=(0,(2,2))))

# ---------- title + honest footer ----------
fig.suptitle("Public skin aging/photoaging datasets nominate the ECM-collagen remodeling axis "
             "as a targetable repair program for hydrogel design",
             fontsize=12.0,fontweight="bold",y=0.975)
...
fig.savefig(OUT/"Figure1_public_ECM_rationale.pdf",bbox_inches="tight")
fig.savefig(OUT/"Figure1_public_ECM_rationale.png",dpi=300,bbox_inches="tight")
print("[written]",OUT/"Figure1_public_ECM_rationale.pdf")
...
print("Panel C terms (label, -log10 padj):",[(lab,round(v,2)) for lab,_,v in cc])
